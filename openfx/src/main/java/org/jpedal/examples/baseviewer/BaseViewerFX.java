...
public class BaseViewerFX extends Application {
    ...
    public Scene setupViewer(final int w, final int h) {
    	try{
    		AnchorPane parentroot = FXMLLoader.load(getClass().getResource("/Hello.fxml"));
            /* 
            ...
            AnchorPane.setTopAnchor(parentroot, 0.0);
			AnchorPane.setBottomAnchor(parentroot, 0.0);
			AnchorPane.setLeftAnchor(parentroot, 0.0);
			AnchorPane.setRightAnchor(parentroot, 0.0);
            parentroot.getChildren().clear();
            parentroot.getChildren().add(root);
            
             scene=new Scene(parentroot, w*FXscaling, h*FXscaling);
