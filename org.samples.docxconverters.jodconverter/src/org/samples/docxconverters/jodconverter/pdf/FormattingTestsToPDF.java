...
public class FormattingTestsToPDF {
	...
	private static void createPDF(OfficeDocumentConverter converter) {
		try {
			...
			converter.convert(new File("docx/FormattingTests.docx"), new File("pdf/FormattingTests.pdf"));
			System.err.println("Generate pdf/FormattingTests.pdf with " + (System.currentTimeMillis() - start) + "ms");
		} catch (Throwable e) {
			e.printStackTrace();
		}
	}
